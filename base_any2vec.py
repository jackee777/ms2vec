...
from numpy import float32 as REAL, ones, random, dtype, zeros, average
...
class BaseWordEmbeddingsModel(BaseAny2VecModel):
    ...
    def __init__(self, sentences=None, workers=3, vector_size=100, epochs=5, callbacks=(), batch_words=10000,
        ...
        **kwargs : object
        ...
        if sentences is not None:
            ...
            #initialize global vector to the average of
            #self.wv.global_vector = average(self.wv.syn0, axis=1)
            #add wordcluster
            self.wordcluster = self.WordCluster()
            #add number of senses
            self.senses = self.Senses_Num()
            self.syn1neg_sense = self.Sense_NS()
            self.train(
    ...
    def build_vocab(self, sentences, update=False, progress_per=10000, keep_raw_vocab=False, trim_rule=None, **kwargs):
        ...
        **kwargs : object
        ...
        self.trainables.prepare_weights(self.hs, self.negative, self.wv, self.max_sense, update=update, vocabulary=self.vocabulary)
    ...
    def estimate_memory(self, vocab_size=None, report=None):
        ...
        report['vectors'] = vocab_size * self.vector_size * dtype(REAL).itemsize * (self.max_sense+1)
        if self.hs:
            report['syn1'] = vocab_size * self.trainables.layer1_size * dtype(REAL).itemsize
        if self.negative:
            report['syn1neg'] = vocab_size * self.trainables.layer1_size * dtype(REAL).itemsize *  (self.max_sense+1)

...
from numpy import exp, dot, zeros, random, dtype, float32 as REAL,\
    ...
    empty, sum as np_sum, ones, logaddexp, log, outer, asarray, \
    append, average, argmax
...
def train_batch_sg(model, sentences, alpha, work=None, compute_loss=False):
    ...
    for sentence in sentences:
        if counts == 5:
            break
        counts += 1
        word_vocabs = asarray([[model.wv.vocab[w], sense] for w, sense in zip(sentence[0], sentence[1]) if w in model.wv.vocab and
                       model.wv.vocab[w].sample_int > model.random.rand() * 2 ** 32]).T
        sense_label = word_vocabs[1]
        word_vocabs = word_vocabs[0]
        for pos, word in enumerate(word_vocabs):
            context_indexes = word_vocabs[pos:pos+model.window]
            if pos == 0:
                average_vector = average(asarray([model.wv.global_vector[context.index] for context in context_indexes]), axis=0)
                sense_vector = asarray([model.wv.syn0[context.index] for context in context_indexes])
                sense_value = dot(sense_vector, average_vector.T)
                sense_value = [argmax(sense_vector[:model.senses[context.index]]) for sense_vector,context in zip(sense_value, context_indexes)]
                sense_label[pos:pos + model.window] = sense_value
            else:
                sense_value = sense_label[pos:pos+model.window]
            if len(word_vocabs) > pos+model.window+1:
                average_vector = average(asarray([model.wv.syn0[context.index][sense] for context,sense in zip(context_indexes, sense_value)]), axis=0)
                sense_vector =  model.wv.syn0[word_vocabs[pos+ model.window+1].index]
                sense_value = dot(sense_vector, average_vector.T)
                sense_value = argmax(sense_value[model.senses[word_vocabs[pos+model.window+1].index]])
                sense_label[pos + model.window+1] = sense_value

            sense1 = sense_label[pos]
            reduced_window = model.random.randint(model.window)  # `b` in the original word2vec code

            # now go over all words from the (reduced) window, predicting each one in turn
            start = max(0, pos - model.window + reduced_window)
            for pos2, word2 in enumerate(word_vocabs[start:(pos + model.window + 1 - reduced_window)], start):
                sense2 = sense_label[pos2]
                # don't train on the `word` itself
                if pos2 != pos:
                    train_sg_pair(
                        model, model.wv.index2word[word.index], word2.index, alpha, sense1, sense2, compute_loss=compute_loss
                    )

            #maybe, the position of to increase the senses is here
...
def train_sg_pair(model, word, context_index, alpha, sense1, sense2,
    ...
    if context_vectors is None:
        if is_ft:
            ...
            context_global_vectors = model.wv.global_vector
            context_sense_vectors = model.wv.syn0
    ...
    if is_ft:
        ...
        l1_global = context_global_vectors[context_index]
        l1_sense = context_sense_vectors[context_index][sense1] # input word (NN input/projection layer)
        lock_factor = context_locks[context_index]

    neu1e = zeros(l1_global.shape)
    ...
    #convert l1 vector to optimized vector
    #content_vector divied into 2 vectors: sense and global
    if model.negative:
        ...
        l2b = model.syn1neg_sense[predict_word.index][sense2]
        l2b = vstack((l2b, model.syn1neg[word_indices]))  # 2d matrix, k+1 x layer1_size
        prod_term = dot(l1_sense.T, l2b.T)
        fb = expit(prod_term)  # propagate hidden -> output
        gb = (model.neg_labels - fb) * alpha  # vector of error gradients multiplied by the learning rate
        if learn_hidden:
            outer_value = outer(gb, l1_sense)
            model.syn1neg_sense[predict_word.index][sense2] += outer_value[0]  # learn hidden -> output
            model.syn1neg[word_indices] += outer_value[1:]
    ...
    if learn_vectors:
        if is_ft:
            ...
            l1_global += neu1e * lock_factor  # learn input -> hidden (mutates model.wv.syn0[word2.index], if that is l1)
            l1_sense += neu1e * lock_factor
    return neu1e
...
class Word2Vec(BaseWordEmbeddingsModel):
    ...
    def __init__(self, sentences=None, size=100, alpha=0.025, window=5, min_count=5,
                 ...
                 max_final_vocab=None, max_sense=3):
        """
    ...
    def WordCluster(self):
        return zeros((len(self.wv.vocab), self.vector_size))

    def Senses_Num(self):
        return ones(len(self.wv.vocab), dtype=int)

    def Sense_NS(self):
        return zeros((len(self.wv.vocab),self.max_sense ,self.layer1_size), dtype=REAL)
    ...
    def save_word2vec_format(self, fname, sense, fvocab=None, binary=False):
        ...
        try:
            ...
            with utils.smart_open(self.source) as fin:
                for j, line in enumerate(itertools.islice(fin, self.limit)):
                    ...
                    i = 0
                    while i < len(line):
                        yield [line[i: i + self.max_sentence_length], self.sense_label[j][i]]
                        i += self.max_sentence_length
    ...
    def scan_vocab(self, sentences, progress_per=10000, trim_rule=None):
        ...
        for sentence_no, sentence in enumerate(sentences):
            if not checked_string_types:
                if isinstance(sentence[0], string_types):
                    logger.warning(
...
class Word2VecTrainables(utils.SaveLoad):
    ...
    def prepare_weights(self, hs, negative, wv, max_sense, update=False, vocabulary=None):
        """Build tables and model weights based on final vocabulary settings."""
        # set initial input/projection and hidden weights
        if not update:
            self.reset_weights(hs, negative, wv, max_sense)
        else:
    ...
    def reset_weights(self, hs, negative, wv, max_sense):
        """Reset all projection weights to an initial (untrained) state, but keep the existing vocabulary."""
        logger.info("resetting layer weights")
        wv.vectors = empty((len(wv.vocab), max_sense, wv.vector_size), dtype=REAL)
        # randomize weights vector by vector, rather than materializing a huge random matrix in RAM at once
        for i in xrange(len(wv.vocab)):
            # construct deterministic seed from word AND seed argument
            wv.vectors[i] = asarray([self.seeded_vector(wv.index2word[i] + str(j) + str(self.seed), wv.vector_size) for j in xrange(max_sense)])
        if hs:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-train", help="Use text data from file TRAIN to train the model", default='text8')
    parser.add_argument("-output", help="Use file OUTPUT to save the resulting word vectors", default='vec.txt')
    parser.add_argument("-window", help="Set max skip length WINDOW between words; default is 5", type=int, default=3)
    parser.add_argument("-size", help="Set size of word vectors; default is 100", type=int, default=5)
    parser.add_argument(
        "-sample",
    ...
    parser.add_argument("-iter", help="Run more training iterations (default 5)", type=int, default=1)
    parser.add_argument(
    ...
    parser.add_argument("-sense", help="the number of sense", type=int, default=3)
    ...
    model = Word2Vec(
        ...
        negative=args.negative, cbow_mean=1, iter=args.iter, max_sense=args.sense
    )

    if args.output:
        outfile = args.output
        model.wv.save_word2vec_format(outfile, model.senses, binary=args.binary)
    else:
        outfile = args.train
        model.save(outfile + '.model')
    """
    if args.binary == 1:
        model.wv.save_word2vec_format(outfile + '.model.bin', args.sense, binary=True)
    else:
        model.wv.save_word2vec_format(outfile + '.model.txt', args.sense, binary=False)
    """

...
def train_batch_sg(model, sentences, alpha, _work, compute_loss):
    ...
    cdef long long vocab_size = len(model.wv.vocab)

    cdef int _compute_loss = (1 if compute_loss else 0)
    cdef REAL_t _running_training_loss = model.running_training_loss

    cdef REAL_t *global_vector = <REAL_t *>(np.PyArray_DATA(model.wv.global_vector))
    cdef REAL_t *neul_vector = <REAL_t *>(np.PyArray_DATA(model.wv.neul_vector))
    #cdef np.float32_t *np_global_vector = <np.float32_t *>(np.PyArray_DATA(model.wv.global_vector))
    cdef REAL_t *syn0 = <REAL_t *>(np.PyArray_DATA(model.wv.vectors))
    cdef np.uint32_t *senses = <np.uint32_t *>(np.PyArray_DATA(model.senses))
    cdef np.float32_t *average_vector = <np.float32_t *>(np.PyArray_DATA(model.wv.average_vector))
    cdef np.float32_t *compare_vector = <np.float32_t *>(np.PyArray_DATA(model.wv.compare_vector))
    cdef REAL_t *word_locks = <REAL_t *>(np.PyArray_DATA(model.trainables.vectors_lockf))
    cdef REAL_t *work
    cdef REAL_t _alpha = alpha
    cdef int size = model.wv.vector_size
    cdef np.int32_t max_vocab_sense = model.max_sense

    cdef int codelens[MAX_SENTENCE_LEN]
    cdef np.uint32_t indexes[MAX_SENTENCE_LEN]
    cdef np.uint32_t sense_label[MAX_SENTENCE_LEN]
    cdef np.uint32_t reduced_windows[MAX_SENTENCE_LEN]
    cdef int sentence_idx[MAX_SENTENCE_LEN + 1]
    cdef int window = model.window

    cdef int i, j, k, l, m, s, s_id
    cdef np.float32_t max_sense, sum_value
    ...
    cdef np.uint32_t sense, sense1, sense2
    cdef np.uint32_t word_index, word2_index
    cdef long long row1, row2, syn0_row1, syn0_row2
    cdef unsigned long long modulo = 281474976710655ULL
    cdef REAL_t f, g, label, f_dot, log_e_f_dot, c_dot
    cdef np.uint32_t target_index
    cdef int d
    ...
    #print("max_vocab_sense", max_vocab_sense)
    ...
    for sent in sentences:
        ...
        for token in sent:
            ...
            sense_label[effective_words] = sense
    ...
    for sent_idx in range(effective_sentences):
        ...
        for i in range(idx_start, idx_end):
            row1 = indexes[i] * size
            for n in range(size):
                compare_vector[n] = compare_vector[n] + (global_vector[row1+n] / (idx_end - idx_start))
        ...
        for i in range(idx_start, idx_end):
            if i == idx_start:
                ...
                for m in range(i, l):
                    row1 = indexes[m] * size
                    for n in range(size):
                        average_vector[n] = average_vector[n] + global_vector[row1+n]
                #print("sense0", senses[0])
                for m in range(i, l):
                    #print("sense", senses[indexes[m]])
                    if senses[indexes[m]] != 1:
                        max_sense = 0
                        s_id = 0
                        for s in range(senses[indexes[m]]):
                            syn0_row1 = indexes[m] * size * max_vocab_sense + s * size
                            sum_value = sum([average_vector[n]*syn0[syn0_row1+n] for n in range(size)])
                            if sum_value > max_sense:
                                max_sense = sum_value
                                s_id = s
                        sense_label[m] = s_id
                    else:
            ...
            if i + window  <= idx_end:
                ...
                for m in range(i, l):
                    ...
                    syn0_row1 = indexes[m] * size * max_vocab_sense + s * size
                    for n in range(size):
                        average_vector[n] = average_vector[n] + syn0[syn0_row1+n]

                if senses[indexes[l]] != 1:
                    max_sense = 0
                    s_id = 0
                    for s in range(senses[indexes[l]]):
                        syn0_row1 = indexes[l] * size * max_vocab_sense + s * size
                        sum_value = sum([average_vector[n]*syn0[syn0_row1+n] for n in range(size)])
                        if sum_value > max_sense:
                            max_sense = sum_value
                            s_id = s
            ...
            with nogil:
                ...
                if k > idx_end:
                    k = idx_end
                sense1 = sense_label[i]
                for j in range(j, k):
                    sense1 = sense_label[j]
                    if j == i:
                        continue
                    if hs:
                    ...
                    if negative:
                        next_random = fast_sentence_sg_neg(
                            negative, cum_table, cum_table_len, syn0, syn1neg, size,
                            indexes[i], indexes[j], _alpha, work, next_random, word_locks,
                            _compute_loss, &_running_training_loss,
                            sense_label[i], sense_label[j], max_vocab_sense,
                            senses, compare_vector)
            """
            j = i - window + reduced_windows[i]
            if j < idx_start:
                j = idx_start
            ...
            for j in range(j, k):
                ...
                if negative:
                    word_index = indexes[i]
                    word2_index = indexes[j]
                    syn0_row1 = word2_index * size * max_vocab_sense + sense1 * size
                    ...
                    for d in range(negative+1):
                        ...
                        if d == 0:
                            f_dot = sum([syn0[syn0_row1 + m]*syn1neg[row2 + m] for m in range(size)])
                            c_dot = sum([syn0[syn0_row1 + m]*compare_vector[m] for m in range(size)])
                            if f_dot < c_dot and senses[word2_index] < max_vocab_sense:
                                senses[word2_index] = senses[word2_index] + 1
                        else:
                            f_dot = sum([syn0[syn0_row1 + m]*syn1neg[row2 + m] for m in range(size)])

                        #print(row1)
                        #print(row2)
                        #print("syn0", syn0[syn0_row1])
                        #print(syn0[row2])
                        #print("f_dot", f_dot)
                        ...
                        if _compute_loss == 1:
                            ...
                            if f_dot <= -MAX_EXP or f_dot >= MAX_EXP:
                                continue
                            log_e_f_dot = LOG_TABLE[<int>((f_dot + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]
                            _running_training_loss = _running_training_loss - log_e_f_dot

                        #print("g", g)
                        """
                        if d==0:
                            for m in range(size):
                                syn1neg[row2 + m] += syn0[syn0_row1 + m] * g
                        else:
                            for m in range(size):
                                syn1neg[row2 + m] += syn0[syn0_row1 + m] * g
                        """
                        for m in range(size):
                            neul_vector[m] += syn1neg[row2 + m] * g

                        for m in range(size):
                            syn1neg[row2 + m] += syn0[syn0_row1 + m] * g

                    for m in range(size):
                        syn0[syn0_row1 + m] += neul_vector[m] * word_locks[word2_index]

                    for m in range(size):
                        global_vector[row1 + m] += neul_vector[m] * word_locks[word2_index]

...
def _save_word2vec_format(fname, vocab, vectors, sense, max_sense, global_vectors, fvocab=None, binary=False, total_vec=None):
        ...
        if len(vectors.shape) == 2:
            if (len(vocab), vector_size) == vectors.shape:
                ...
                vectors = vectors.reshape((len(vocab), max_sense, vector_size))
                with utils.smart_open(fname, 'wb') as fout:
                    fout.write(utils.to_utf8("%s %s %s\n" % (total_vec, max_sense, vector_size)))
                    # store in sorted order: most frequent words at the top
                    for word, vocab_ in sorted(iteritems(vocab), key=lambda item: -item[1].count):
                        row_vocab = vectors[vocab_.index]
                        if binary:
                            row = row.astype(REAL)
                            fout.write(utils.to_utf8(word) + b" " + row.tostring())
                        else:
                            for i in range(sense[vocab_.index]):
                                row = row_vocab[i]
                                if type(row) != type(list()):
                                    row = list(row)
                                fout.write(utils.to_utf8("%s %s\n" % (word+"--"+str(i), ' '.join(repr(val) for val in row))))
                with utils.smart_open("global"+fname, 'wb') as fout:
                    fout.write(utils.to_utf8("%s %s\n" % (total_vec, vector_size)))
                    # store in sorted order: most frequent words at the top
                    for word, vocab_ in sorted(iteritems(vocab), key=lambda item: -item[1].count):
                        row = global_vectors[vocab_.index]
                        if type(row) != type(list()):
                            row = row.tolist()
                        if binary:
                            row = row.astype(REAL)
            ...
            max_sense = vectors.shape[1]
            vector_size = vectors.shape[2]
            assert (len(vocab), max_sense, vector_size) == vectors.shape
            with utils.smart_open(fname, 'wb') as fout:
                fout.write(utils.to_utf8("%s %s %s\n" % (total_vec, max_sense, vector_size)))
                # store in sorted order: most frequent words at the top
                for word, vocab_ in sorted(iteritems(vocab), key=lambda item: -item[1].count):

...
class Word2Vec(BaseWordEmbeddingsModel):
    ...
    def save_word2vec_format(self, fname, sense, max_sense, fvocab=None, binary=False):
        ...
        try:
            ...
            with utils.smart_open(self.source) as fin:
                for j, line in enumerate(itertools.islice(fin, self.limit)):
                    ...
                    i = 0
                    while i < len(line):
                        sum_line = [[word, sense] for word,sense in
                                    zip(line[i: i + self.max_sentence_length], self.sense_label[j][i])]
                        yield sum_line
                        i += self.max_sentence_length
    ...
    def scan_vocab(self, sentences, progress_per=10000, trim_rule=None):
        ...
        for sentence_no, sentence in enumerate(sentences):
            if not checked_string_types:
                if isinstance(sentence[0][0], string_types):
                    logger.warning(
...
class Word2VecTrainables(utils.SaveLoad):
    ...
    def reset_weights(self, hs, negative, wv, max_sense):
        ...
        # reshape vector_size. it is very important
        wv.vectors = wv.vectors.reshape((len(wv.vocab) * max_sense, wv.vector_size))
        wv.global_vector = wv.vectors[::max_sense]
        wv.average_vector = zeros(wv.vector_size)
        wv.compare_vector = zeros(wv.vector_size)
        wv.neul_vector = zeros(wv.vector_size)
        print(wv.vectors[0])
        if hs:
